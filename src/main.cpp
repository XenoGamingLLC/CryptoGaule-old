...
uint256 hashGenesisBlock("0xef84ae326e8b09bb7d4ff5d24d54f435cabb72cece85b771c2bf3d6cc3b6d9a4");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // CryptoGaule: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "CryptoGaule Signed Message:\n";
...
{
    // CryptoGaule: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // CryptoGaule: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // CryptoGaule: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1515842425, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = " January 8 2018 Cryptocurrency Markets Tank After Signs China ";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 500 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04a5ed6d1adddedea1c6e5563141fcd863aaae2487e3562990a5b82fd0ed088ff54c43730d2690c9b3cffe0e265762cdb5d95ca4fb835ed28d7929a258977bbed8") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1515842425;
        ...
        assert(block.hashMerkleRoot == uint256("0x0a74f1344aab6dad7934ad3dd181e5df06430801634620f8b91240c1dda4c547"));
...
unsigned char pchMessageStart[4] = { 0x6c, 0xf8, 0x2e, 0x8f }; // CryptoGaule: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CryptoGauleMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CryptoGauleMiner(CWallet *pwallet)
{
    printf("CryptoGauleMiner started\n");
    ...
    try { loop {
        ...
        printf("Running CryptoGauleMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CryptoGauleMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CryptoGauleMiner, pwallet));
}
